/* ƒана строка текста, в которой слова разделены пробелами и зап€тыми. Ќеобходимо:

      а) определить количество слов в строке, которые начинаютс€ с согласной буквы;

      б) найти слова, содержащее две одинаковые буквы подр€д и их пор€дковые номера;

      в) вывести слова в алфавитном пор€дке.

—троку инициализировать в коде программы:
...
void func_1()
{
	...
	cout << " оличество слов в строке, которые начинаютс€ с согласной буквы: ";
...
{
	int i,count(0);
	cout << "ѕор€дковые номера слов, содержащих две одинаковые буквы подр€д: ";
	for(i=0;i<sizeof(str);i++)
	{
...
void func_3()
{
	int l = strlen(str);
	int k = 0;
	cout << "—трока, отсортированна€ по алфавиту:";
	for(int i=0;i<l;i++)
